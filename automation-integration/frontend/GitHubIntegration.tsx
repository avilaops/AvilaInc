    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="flex border-b">
          <button onClick={() => setActiveSection('overview')} className={`px-6 py-3 font-medium transition-colors ${
            activeSection === 'overview' ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700' : 'text-gray-600 hover:bg-gray-50'
          }`}>📊 Visão Geral</button>
          <button onClick={() => setActiveSection('issues')} className={`px-6 py-3 font-medium transition-colors ${
            activeSection === 'issues' ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700' : 'text-gray-600 hover:bg-gray-50'
          }`}>🐛 Issues</button>
          <button onClick={() => setActiveSection('prs')} className={`px-6 py-3 font-medium transition-colors ${
            activeSection === 'prs' ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700' : 'text-gray-600 hover:bg-gray-50'
          }`}>🔀 Pull Requests</button>
          <button onClick={() => setActiveSection('branches')} className={`px-6 py-3 font-medium transition-colors ${
            activeSection === 'branches' ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700' : 'text-gray-600 hover:bg-gray-50'
          }`}>🌿 Branches</button>
          <button onClick={() => setActiveSection('files')} className={`px-6 py-3 font-medium transition-colors ${
            activeSection === 'files' ? 'bg-purple-50 text-purple-700 border-b-2 border-purple-700' : 'text-gray-600 hover:bg-gray-50'
          }`}>📁 Arquivos</button>
        </div>

        <div className="p-6">
          {error && <div className="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded">⚠️ {error}</div>}
          {loading && <div className="text-center py-8"><div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-purple-700"></div><p className="mt-2 text-gray-600">Carregando...</p></div>}
          {!loading && activeSection === 'overview' && repository && <div className="space-y-4"><div className="prose max-w-none"><h3 className="text-lg font-semibold">Sobre o Repositório</h3><p className="text-gray-600">{repository.description || 'Sem descrição disponível'}</p></div></div>}
        </div>
      </div>
    </div>
  );
