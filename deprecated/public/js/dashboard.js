...
async function loadAIData() {
    ...
    try {
        // Carregar dados de uso, modelos e status
        const [usageResponse, modelsResponse, statusResponse] = await Promise.all([
            fetch(`${API_BASE}/openai/usage`),
            fetch(`${API_BASE}/openai/models`),
            fetch(`${API_BASE}/openai/status`)
        ]);
        ...
        if (!statusData.success) throw new Error('OpenAI não configurado');
        
        let html = '<div class="stats-grid">';
        
        // Status da configuração
        html += `<div class="stat-card">
            <div class="stat-icon">${statusData.configured ? '✅' : '❌'}</div>
            <div class="stat-content">
                <h3>OpenAI</h3>
                <p class="stat-value">${statusData.configured ? 'Configurado' : 'Não configurado'}</p>
                <p class="stat-label">Status da API</p>
            </div>
        </div>`;
        ...
        if (modelsData.success) {
            html += `<div class="stat-card">
                ...
                <div class="stat-content">
                    <h3>Modelos GPT</h3>
                    <p class="stat-value">${modelsData.models.length}</p>
        ...
        if (usageData.success) {
            html += `<div class="stat-card">
                ...
                <div class="stat-content">
                    <h3>Uso da API</h3>
                    <p class="stat-value">${usageData.total}</p>
                    <p class="stat-label">Requisições (30d)</p>
                </div>
            </div>`;
        }
        ...
        if (modelsData.success && modelsData.models.length > 0) {
            html += '<h3 style="margin: 30px 0 20px 0;">🤖 Modelos OpenAI Disponíveis</h3>';
            
            modelsData.models.slice(0, 12).forEach(model => {
                const modelDate = new Date(model.created * 1000).toLocaleDateString('pt-BR');
                html += `
                    <div class="service-card">
                        <div class="service-header">
                            <div>
                                <div class="service-title">${model.id}</div>
                                <p style="color: #64748b; margin-top: 5px;">Criado: ${modelDate} | Por: ${model.owned_by}</p>
                            </div>
        ...
        html += `
            <div style="margin-top: 30px;">
                <h3 style="margin-bottom: 15px;">⚡ Funcionalidades OpenAI Integradas</h3>
                <div class="stats-grid">
                    <div class="stat-card">
                        <div class="stat-icon">💬</div>
                        <div class="stat-content">
                            <h4>Chat Completion</h4>
                            <p>Conversas com GPT-3.5/4</p>
                        </div>
                    </div>
                    <div class="stat-card">
                        <div class="stat-icon">🎨</div>
        ...
        // Outros serviços de AI
        html += `
            <div style="margin-top: 30px;">
                <h3 style="margin-bottom: 15px;">🔑 Outros Serviços de AI</h3>
                <div class="service-card">
                    <p><strong>🌊 DeepSeek:</strong> ${process.env.DEEPSEEK_API_KEY ? 'Token configurado' : 'Não configurado'}</p>
                    <p style="margin-top: 10px;"><strong>🤗 HuggingFace:</strong> ${process.env.HF_TOKEN ? 'Token configurado' : 'Não configurado'}</p>
                    <p style="margin-top: 10px;"><strong>🦙 Ollama:</strong> ${process.env.OLLAMA_API_KEY ? 'Token configurado' : 'Não configurado'}</p>
                    <p style="margin-top: 10px;"><strong>🔗 LangSmith:</strong> ${process.env.LANGSMITH_API_KEY ? 'API Key configurada' : 'Não configurada'}</p>
                </div>
                <div style="margin-top: 15px;">
        ...
        container.innerHTML = `<div class="service-card" style="background: #fee2e2; color: #991b1b;">
            <p><strong>Erro ao carregar dados da OpenAI:</strong> ${error.message}</p>
            <p style="margin-top: 10px;">Verifique se a OPENAI_API_KEY está configurada no arquivo .env</p>
        </div>`;
    }
}
...
function openSettings() {
    alert('Configurações em desenvolvimento. Use o menu lateral para navegar entre as seções.');
}
