...
public class GooglePlacesController : ControllerBase
{
    ...
    private static CompanyDto MapToDto(Company c) => new()
    {
        ...
        UpdatedAt = c.UpdatedAt
                ...
                if (await _companyRepository.ExistsByGooglePlaceIdAsync(placeId))
                {
                    _logger.LogInformation("Company with Google Place ID {PlaceId} already exists", placeId);
                    continue;
                }
                ...
                {
                    _logger.LogWarning("Could not get details for place ID {PlaceId}", placeId);
                    continue;
                }
                ...
                var company = new Company
                {
                    ...
                    Location = placeDetails.Latitude.HasValue && placeDetails.Longitude.HasValue
                        ? new NetTopologySuite.Geometries.Point(placeDetails.Longitude.Value, placeDetails.Latitude.Value)
                        : null,
                ...
                _logger.LogInformation("Imported company: {Name} ({PlaceId})", company.Name, company.GooglePlaceId);
            }

            var dtos = importedCompanies.Select(MapToDto);
            return Ok(dtos);
        }
        catch (Exception ex)
        {
    ...
    private static CompanyDto MapToDto(Company company)
    {
        return new CompanyDto
        {
            ...
            Latitude = company.Location?.Y,
            Longitude = company.Location?.X,
            Phone = company.Phone,
            Website = company.Website,
            Types = company.Types,
            Rating = company.Rating,
            UserRatingsTotal = company.UserRatingsTotal,
            BusinessStatus = company.BusinessStatus,
            LastSyncedAt = company.LastSyncedAt,
            Cnpj = company.Cnpj,
            LastCnpjLookupAt = company.LastCnpjLookupAt,
